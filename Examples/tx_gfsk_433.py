# =============================================================================
# GFSK Transmitter — NUCLEO-WL55 @ 433.5 MHz
# 9600 bps, Gaussian BT=0.5, Fdev=5 kHz, fixed 10-byte payload
# Air time ~16 ms/packet — transmits continuously
# Upload: mpremote cp tx_gfsk_433.py :main.py && mpremote reset
# =============================================================================
...
def init():
    _base_init()
    _cmd(bytes([_SET_PKT_TYPE, 0x00]))              # GFSK
    _cmd(bytes([_SET_MOD_PARAMS])
         + _bitrate_word(9600)                      # 9600 bps
         + bytes([0x09])                            # Gaussian BT=0.5
         + bytes([0x0D])                            # RxBW 29.3 kHz
         + _fdev_word(5000))                        # Fdev 5 kHz
    # Preamble=32b, Detect=8b, Sync=24b, NoAddr, Fixed, Payload=10, CRC2B, NoWhiten
    _cmd(bytes([_SET_PKT_PARAMS,
                0x00,0x20,0x05,0x18,0x00,0x01,0x0A,0x02,0x00]))
    print("GFSK ready: 9600bps BT=0.5 Fdev=5kHz @ 433.500 MHz  -17 dBm")
...
def main():
    ...
    while True:
        payload = b"GF433:" + n.to_bytes(4, "big")  # 10 bytes
        _send(payload, 30)
